...
public:
    ...
    void    randomize(unsigned int nd_cnt = 0, float density = 0,     // Generate a random graph structure base on
                      cost_type cost_low = 1.0, cost_type cost_high = 10.0);  // node count and edge density.
...
private:
    void    delete_edge_uni(node src, node dst);
    void    set_edge_value_uni(node src, node dst, cost_type cost);
    void    add_random_edge(node src, node dst, float density, cost_type cost_low, cost_type cost_high);
    inline  cost_type random_cost(cost_type low, cost_type high) const;
    ...
    vector<cost_type> node_values;          // Nodes Values
